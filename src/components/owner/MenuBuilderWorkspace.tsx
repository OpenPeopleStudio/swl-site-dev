...
export function ComponentBuilder({ components }: ComponentBuilderProps) {
  return (
    <section className="rounded-[24px] border border-white/10 bg-white/5 p-4 shadow-[0_25px_70px_rgba(0,0,0,0.35)]">
          ...
          >
            ...
            {component.estimatedTime && (
              <p className="text-xs text-white/50">
                {component.estimatedTime} · supervised
              </p>
            )}
          </article>
        ))}
...
export function IngredientPicker({ ingredients }: IngredientPickerProps) {
  return (
    <section className="rounded-[24px] border border-white/10 bg-white/5 p-4 text-sm text-white shadow-[0_25px_70px_rgba(0,0,0,0.35)]">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-xs uppercase tracking-[0.4em] text-white/50">
          Ingredients
        </p>
        <button
          type="button"
          className="rounded-full border border-white/20 px-3 py-1 text-xs uppercase tracking-[0.4em] text-white/70"
        >
          Link Inventory
        </button>
      </div>
        ...
        {ingredients.map((ingredient) => (
          ...
          >
            <div className="flex items-center justify-between">
              <p>{ingredient.name}</p>
              {ingredient.cost && (
                <span className="text-xs text-white/60">${ingredient.cost}</span>
              )}
            </div>
            <p className="text-[11px] uppercase tracking-[0.3em] text-white/50">
              {ingredient.vendor ?? "Unassigned"}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {entries.map((entry) => (
          ...
          >
            ...
            {entry.deltaCost && (
              <p className="text-xs text-white/50">Δ {entry.deltaCost}</p>
            )}
          </article>
        ))}
      </div>
