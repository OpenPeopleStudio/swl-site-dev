...
export function FoodVendorPanel({ vendors }: FoodVendorPanelProps) {
  return (
    <section className="rounded-3xl border border-white/10 bg-white/5 p-5 text-white shadow-[0_20px_60px_rgba(0,0,0,0.45)]">
      <header className="flex items-center justify-between">
        <div>
          ...
          <h2 className="text-xl font-light">Consistency Pulse</h2>
        </div>
      </header>
        ...
        {vendors.map((vendor) => (
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
              <p className="font-semibold tracking-[0.3em] uppercase text-white/70">
                {vendor.name}
              </p>
