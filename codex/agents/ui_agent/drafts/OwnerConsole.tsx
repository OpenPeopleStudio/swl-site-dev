...
const MOCK_PANELS: Panel[] = [
  ...
  },
  {
    ...
    description: "Deliveries · alerts · reliability.",
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Weather", value: "Calm · on-time arrivals expected" },
    ],
  },
];
  ...
  return (
    <div className="relative flex min-h-screen flex-col gap-10 bg-[#02030a] px-6 pb-24 pt-12 text-white">
      <header className="text-center">
        <p className="text-[0.65rem] uppercase tracking-[0.5em] text-white/40">
          Owner Console
        </p>
      ...
      {menuBuilderOpen && (
        <MenuBuilderSheet onClose={() => setMenuBuilderOpen(false)} />
      )}
    </div>
  );
}
      ...
      <div className="mt-4 divide-y divide-white/10 text-sm text-white/75">
        {panel.items.map((item) => (
          <div key={item.label} className="flex items-start justify-between gap-4 py-3">
            ...
            <p
              className={`text-sm ${
                item.tone === "warn" ? "text-amber-200" : "text-white"
              }`}
            >
              {item.value}
            </p>
